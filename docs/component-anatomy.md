...
| `MetaStrip`  | Compact semantic key/value facts in inline or stacked form.                 | `meta-strip`, `meta-strip-item`, `meta-strip-label`, `meta-strip-value`, `meta-strip-caption`                                   | Use instead of a raw `<dl>` or ad hoc metadata grid.                                                            | Do not use for prominent dashboard statistics; use `Stat`.              |
...
| `Skeleton`       | Loading placeholder.                                                    | `skeleton`                                                                                                         | None                                                                                                        | Give it explicit dimensions with `width` and `height`, or through a layout parent. | Do not render meaningful content inside skeletons.                                        |
...
| `CommandPalette` | Dialog slots plus `command`, `command-header`, `command-input`, `command-list`, and `command-item`                | `data-command-palette-content`, `data-command-palette-overlay`                                  | Compose search and command launchers from the shared dialog behavior.   | Put the input before result controls for deterministic initial focus. |
