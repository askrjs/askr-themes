import type { ResolvedPair } from "./scenarios/contrast";
...
test.describe("computed WCAG contrast", () => {
  for (const theme of THEMES) {
    test(`should keep ${theme} text and control colors readable`, async ({ render, run }) => {
      await render();

      const pairs =
        theme === "light" || theme === "dark" ? [...PAIRS, ...DEFAULT_THEME_PAIRS] : PAIRS;
      const resolved = await run<ResolvedPair[]>(
        "resolve",
        theme,
        pairs.map(([foregroundToken, backgroundToken]) => [foregroundToken, backgroundToken]),
      );

      for (const [index, [foregroundToken, backgroundToken, minimum]] of pairs.entries()) {
        // Playwright has no per-assertion message, so the pair the original
        // named in `expect(value, message)` becomes the enclosing step title.
        await test.step(`${theme}: ${foregroundToken} on ${backgroundToken}`, () => {
          const measured = resolved[index]!;
