...
describe("components entrypoint", () => {
  ...
  it("should renders new catalog-only anatomy with stable slots", () => {
    expect(asElement(AlertTitle({ children: "Heads up" })).props["data-slot"]).toBe(
      "alert-title",
    );
    expect(asElement(AlertDescription({ children: "Details" })).props["data-slot"]).toBe(
      "alert-description",
    );
    expect(asElement(Brand({ children: [BrandMark({}), BrandLabel({ children: "Brand" })] })).props[
      "data-slot"
    ]).toBe("brand");
    ...
    expect(asElement(BreadcrumbItem({ children: BreadcrumbLink({ href: "/", children: "Home" }) }))
      .props["data-slot"]).toBe("breadcrumb-item");
    expect(asElement(CardHeader({ children: CardTitle({ children: "Usage" }) })).props[
      "data-slot"
    ]).toBe("card-header");
    expect(asElement(CardContent({ children: "Body" })).props["data-slot"]).toBe("card-content");
    expect(asElement(TabsList({ children: TabsTrigger({ children: "Preview" }) })).props[
      "data-slot"
    ]).toBe("tabs-list");
    expect(asElement(TabsContent({ children: "Panel" })).props["data-slot"]).toBe(
      "tabs-content",
    );
    ...
    expect(asElement(CommandItem({ children: "Calendar", selected: true })).props[
      "aria-selected"
    ]).toBe("true");
    expect(asElement(ItemGroup({ children: ItemHeader({ children: "Header" }) })).props[
      "data-slot"
    ]).toBe("item-group");
    ...
    expect(asElement(SheetHeader({ children: SheetTitle({ children: "Settings" }) })).props[
      "data-slot"
    ]).toBe("sheet-header");
    ...
    expect(asElement(SidebarProvider({ children: SidebarInset({ children: "Main" }) })).props[
      "data-slot"
    ...
    expect(asElement(SidebarMenuButton({ children: "Dashboard", active: true })).props[
      "data-active"
    ]).toBe("true");
