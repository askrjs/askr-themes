...
function extractBlockBody(css: string, selectorPattern: RegExp): string {
  ...
  if (!match || match[1] === undefined) {
    throw new Error(`Unable to find CSS block for ${selectorPattern}`);
  }

  return match[1];
}

function normalizeCssBlock(body: string): string {
...
describe("token completeness", () => {
  ...
  it("should define every design token referenced by default components in default tokens.css", () => {
    ...
    const isLayoutRuntimeVar = (token: string) =>
      /^--ak-(display|m|mx|my|mt|mr|mb|ml|p|px|py|pt|pr|pb|pl|width|min-width|max-width|height|min-height|max-height|position|inset|top|right|bottom|left|overflow|overflow-x|overflow-y|flex-basis|flex-grow|flex-shrink|grid-area|grid-column|grid-column-start|grid-column-end|grid-row|grid-row-start|grid-row-end|gap|flex-direction|align-items|justify-content|flex-wrap|grid-template-areas|grid-template-columns|grid-template-rows|grid-auto-flow|border|border-top|border-bottom|border-left|border-right|border-radius|background|box-shadow)-(base|sm|md|lg|xl)$/.test(
        token,
      );
  ...
  it("should keep the template token contract aligned with the default theme", () => {
    const defaultCss = readFileSync(TOKENS_FILE, "utf-8");
    const templateCss = readFileSync(TEMPLATE_TOKENS_FILE, "utf-8");
    const defaultSets = getThemeTokenSets("default");
    const templateRootTokens = extractTokensForSelector(templateCss, isRootSelector);
    const templateLightTokens = extractTokensForSelector(templateCss, isLightSelector);
    const templateDarkTokens = extractTokensForSelector(templateCss, isDarkSelector);

    expect([...templateRootTokens].sort()).toEqual([...defaultSets.rootTokens].sort());
    expect([...templateLightTokens].sort()).toEqual([...defaultSets.lightTokens].sort());
    expect([...templateDarkTokens].sort()).toEqual([...defaultSets.darkTokens].sort());
    expect(
